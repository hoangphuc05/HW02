...
private:
	...
	{
	private:
		std::string name; //The name of the individual represented in the entry.
		std::string number; //The phone number for this individual.

	public:
		Directory_Entry() {} // Default no-argument constructor 

		Directory_Entry(std::string the_name, std::string the_number) {
			//Contructor with the name and number
			name = the_name;
			number = the_number;
		}
